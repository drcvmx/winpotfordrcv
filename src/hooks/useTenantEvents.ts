...
export function useTenantEvents(tenantId: string) {
  return useQuery({
    ...
    queryFn: async () => {
      // DEMO MODE
      const data = demoData.getTenantEvents(tenantId);
...
export function useUpsertTenantEvent() {
  ...
  return useMutation({
    mutationFn: async (input: TenantEventInput) => {
      ...
      const currentEvents = demoData.getTenantEvents(input.tenant_id);

      if (input.id) {
        // Update existing
        const index = currentEvents.findIndex((e: any) => e.id === input.id);
        ...
        demoData.setTenantEvents(input.tenant_id, currentEvents);
        return updatedEvent;
      } else {
        // Insert new
        const newEvent = {
...
export function useDeleteTenantEvent() {
  ...
  return useMutation({
    mutationFn: async ({ id, tenantId }: { id: string; tenantId: string }) => {
      const currentEvents = demoData.getTenantEvents(tenantId);
      const filtered = currentEvents.filter((e: any) => e.id !== id);
      demoData.setTenantEvents(tenantId, filtered);
      return { id, tenantId };
    },
    onSuccess: (variables) => {
      ...
      toast({
        ...
        description: "El evento ha sido eliminado (Modo Demo).",
        className: "bg-green-600 text-white border-none",
      });
    },
    onError: (error) => {
